...
$drivers = $pdo->query("
    SELECT u.user_id, u.full_name, u.phone, u.account_status,
           IFNULL((SELECT delivery_status FROM delivery WHERE driver_id = u.user_id AND delivery_status != 'delivered' LIMIT 1), 'Available') as current_status
    FROM user u
...
<p class="subtitle">Driver Fleet Management</p>
<div class="screen">
  <div class="navbar">
  ...
  <div class="content">
    <div class="sidebar-layout">
      <div class="side-menu">
        ...
        <a href="admin_drivers.php" style="text-decoration: none;"><div class="side-item active">Drivers</div></a>
        <a href="admin_customers.php" style="text-decoration: none;"><div class="side-item">Customers</div></a>
      </div>
      <div>
        <div class="card">
          <div class="card-title">Fleet Roster</div>
          <table>
            <thead>
              <tr><th>Driver ID</th><th>Name</th><th>Phone Number</th><th>Delivery Status</th></tr>
            </thead>
            <tbody>
              <?php
              if (count($drivers) > 0) {
